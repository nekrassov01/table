...
// BenchmarkMarkdownTableTransformerValue isolates callback input costs.
// Callbacks return existing strings to exclude formatting allocations.
func BenchmarkMarkdownTableTransformerValue(b *testing.B) {
	...
	t := markdown.NewTable(io.Discard,
		markdown.WithHeader([]string{"MESSAGE", "LEVEL"}),
		markdown.WithTransformer(markdown.Columns(0), func(v table.Value) (string, *markdown.Color, *markdown.Decoration) {
			return messageText(v), nil, nil
		}),
		markdown.WithTransformer(markdown.Columns(1), func(v table.Value) (string, *markdown.Color, *markdown.Decoration) {
...
// BenchmarkMarkdownStreamTransformerValue isolates callback input costs.
// Callbacks return existing strings to exclude formatting allocations.
func BenchmarkMarkdownStreamTransformerValue(b *testing.B) {
	message := "request completed"
	const rowCount = 1000
	messageText := func(v table.Value) string {
		return v.AsString()
	...
	b.ReportAllocs()
	for b.Loop() {
		s := markdown.NewStream(io.Discard,
			markdown.WithHeader([]string{"MESSAGE", "LEVEL"}),
			markdown.WithTransformer(markdown.Columns(0), func(v table.Value) (string, *markdown.Color, *markdown.Decoration) {
				return messageText(v), nil, nil
			}),
