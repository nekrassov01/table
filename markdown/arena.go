...
func (o *arena) newConfig(option *option, bodyRows int) config {
	return config{
		state: &o.config,
		output: configResult{
			option:   option,
...
func (o *arena) resumeConfig(option *option, bodyRows int) config {
	state := &o.config
	return config{
		state: state,
		output: configResult{
...
// resumeCompiler continues compilation with body span state retained from
// prior rows.
...
// newSolver starts the column-geometry pass.
...
type solverState struct {
	columnMetrics []columnMetric // Measured metrics in logical column order.
}
