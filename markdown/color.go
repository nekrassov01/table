...
// NewColor returns a Color that wraps values in an HTML span using fg and bg
// as CSS color values. An empty value omits that property; if both are empty,
...
func NewColor(fg, bg string) *Color {
	...
	// Both values are inserted into a double-quoted style attribute and must
	// therefore use attribute escaping.
	fg = escapeAttr(fg)
	bg = escapeAttr(bg)
