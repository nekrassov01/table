...
func (o *Set[T]) Resolve(columns []T, columnCount int, defaults T) []T {
	if o.state == nil {
		return o.resolveValues(columns, columnCount, defaults)
	}
	return o.resolveState(columns, columnCount, defaults)
}

// resolveValues applies defaults and contiguous explicit settings.
func (o *Set[T]) resolveValues(columns []T, columnCount int, defaults T) []T {
	...
	copy(columns, o.Values)
	return columns
}

// resolveState resolves Values using state defaults when present, then applies sparse settings.
func (o *Set[T]) resolveState(columns []T, columnCount int, defaults T) []T {
	state := o.state
	inputDefaults := defaults
	if state.hasDefaults {
		inputDefaults = state.defaults
	}
	columns = o.resolveValues(columns, columnCount, inputDefaults)
	for _, value := range state.values {
		if value.index >= len(columns) {
			break
		}
		columns[value.index] = value.config
	}
	return columns
}
