...
6. Use the audit's option inventory and uncovered pairs as the mechanical input. Investigate uncovered pairs in this order: geometry against geometry, such as width, padding, truncation, spans, and terminal fitting; then geometry against markup; then the remaining pairs. Do not equate an uncovered pair with a defect.
