...
5. Run `go run ./.agents/skills/check-baseline/scripts` with the resolved revision, benchmark target, `benchtime`, and `count`. Use `--bench` instead of `--target` for a focused measurement. The script records the environment, creates a detached worktree, runs one sample per process without profiling, alternates which revision runs first in each sample pair, runs `benchstat`, reports the median of each metric, and removes its temporary worktree and output. Add `--keep` only when raw output is needed for investigation, then remove the reported artifact directory afterward.
6. Inspect the `benchstat` comparison for variance, statistical significance, and direction. Use the script's per-benchmark medians to interpret allocation counts and allocated bytes, not as an exact ns/op acceptance threshold.
...
8. Explain each difference before reporting it. Attribute an allocation difference with escape-analysis evidence. Attribute a byte difference to arena growth, pool state, benchmark order, or a caller-provided closure. Treat an insignificant ns/op difference as measurement noise. Confirm a significant slowdown with an independent execution under the same conditions and reject it only when the direction and significance reproduce. Increase `benchtime` and `count` when results do not converge; never select only a favorable run.
...
- Do not leave worktrees, benchmark output, or temporary source changes behind.
