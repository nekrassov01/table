...
5. Run `go run .agents/skills/check-baseline/scripts/compare.go` with the resolved revision, benchmark target, `benchtime`, and `count`. Use `--bench` instead of `--target` for a focused rerun. The script records the environment, creates a detached worktree, measures both revisions, runs `benchstat`, extracts raw minima, and removes its temporary worktree, output, and profiles. Add `--keep` only when profiles are needed to investigate a confirmed regression, then remove the reported artifact directory after the investigation.
6. Inspect the `benchstat` comparison for variance and direction. Use the script's raw minima for ns/op because the representative value reported by `benchstat` is not the acceptance value.
7. Apply the criteria in priority order: allocs/op first, then the minimum ns/op, then B/op. Confirm that every affected `Table` `Reuse` benchmark is still at 0 allocs/op.
...
- Do not present a mean or a median as evidence for ns/op.
